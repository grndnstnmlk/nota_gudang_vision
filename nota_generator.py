...
def build_nota_sheet(wb,title,rows,nama,alamat,tanggal):
    ...
    footer=[("JUMLAH ",sum_formula,"#,##0",False),
            ("PPH 1%",f"=CEILING({L_jml}{r_jml}*0.01,5000)","#,##0",False)]
    if has_gt:
...
OUT_DIR = "nota"   # semua hasil generate disimpan di subfolder ini

def default_out_name(notas):
    """Nama file hasil otomatis: nota_<awal-akhir>. Beberapa nota digabung dgn '_'.
    Contoh: [Nota(142,147)] -> 'nota_142-147.xlsx';
            [Nota(142,147),Nota(148,163)] -> 'nota_142-147_148-163.xlsx'."""
    parts="_".join(f"{n.no_start}-{n.no_end}" for n in notas)
    return f"nota_{parts}.xlsx"

_ILLEGAL_SHEET = re.compile(r"[\\/?*\[\]:]")
def _safe_title(s):
    """Bersihkan judul sheet dari karakter yang dilarang Excel (\\ / ? * [ ] :),
    buang apostrof di tepi, dan potong maksimal 31 karakter."""
...
def build_workbook(src_path,out_path,notas):
    ...
    # Taruh hasil di subfolder "nota" (dibuat otomatis) BILA out_path belum menyertakan folder.
    # Kalau memberi -o dengan folder sendiri (mis. "lain/Hasil.xlsx"), folder itu yang dipakai.
...
# ============================================================ CARA EKSEKUSI ============================================================
# Pola pakai lewat baris perintah (Terminal / CMD):
#   python nota_generator B.py "Buku_Soter_1-1000 GREEND.xlsx" 142-147
#       -> hasil OTOMATIS disimpan di:  nota/nota_142-147.xlsx
#   python nota_generator B.py "Buku_Soter_1-1000 GREEND.xlsx" 148-163 164-177
#       -> nota/nota_148-163_164-177.xlsx
# Tiap "142-147" = satu nota (satu sheet). Boleh beberapa sekaligus, dipisah spasi.
# SEMUA hasil masuk ke subfolder "nota" (dibuat otomatis di folder yang sama).
# (Opsional) kalau mau menentukan nama file sendiri, pakai: -o NamaFile.xlsx
#   python nota_generator B.py "Buku_Soter_1-1000 GREEND.xlsx" -o Hasil.xlsx 142-147 148-163
#       -> nota/Hasil.xlsx  (kalau -o memuat folder sendiri, folder itu yang dipakai)
if __name__ == "__main__":
    ...
    if args:
        ...
        rest = args[1:]
        out = None
        # ambil opsi -o NamaFile.xlsx kalau ada
        if "-o" in rest:
            i = rest.index("-o")
            out = rest[i+1]
        ...
        if not notas:
            print("Pemakaian: python nota_generator B.py <file_xlsx1> [-o Hasil.xlsx] 142-147 [148-163 ...]")
            sys.exit(1)
        out = build_workbook(src, out, notas)   # out=None -> nota_<rentang>.xlsx
        print(f"Selesai -> {out}  ({len(notas)} nota)")
    else:
        # ---- MODE MUDAH: edit daftar di bawah lalu jalankan "python nota_generator B.py" ----
        SRC = "Buku_Soter_1-1000 GREEND.xlsx"   # file xlsx 1 (Buku Sortir)
        OUT = None   # None = nama file hasil OTOMATIS (nota_<rentang>.xlsx).
                     # Isi manual kalau mau, contoh: OUT = "Hasil.xlsx"
        # Nama, tanggal, alamat, DAN GT otomatis diambil dari xlsx 1.
        # Susunan kolom NAMA di xlsx 1 (atas->bawah): nama -> tanggal -> alamat.
        DAFTAR_NOTA = [
            Nota(1, 35),   # <- cukup isi NO awal & NO akhir; sisanya otomatis
            # Nota(36, 47),
            # --- kalau mau menimpa manual (opsional) ---
            # Nota(36, 47, alamat="KADUR"),
            # Nota(1, 35, nama="H. GHALIB", tanggal="15 Agustus 2026"),
        ]
